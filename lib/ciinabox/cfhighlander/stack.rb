  ...
  module CfHiglander
    class Stack
      def initialize(ciinabox_config, output_dir)
        @ciinabox_component = "ciinabox-#{ciinabox_config['ciinabox_name']}"
      ...
      def create()
        ...
        create_stack(template_url, @region)
      end
  
      private
  
      def create_stack(template_url, region)
        stack_name = "#{@ciinabox_component}"
      ...
      def publish_cf_templates(cf_compiler)
        publisher = Cfhighlander::Publisher::ComponentPublisher.new(cf_compiler.component, false)
