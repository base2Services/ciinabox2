  ...
  module CfHiglander
    class Utils
      ...
      def self.apply_change_set(stack_name, change_set, change_set_type, region)
        cloudformation = Aws::CloudFormation::Client.new(region: region)
        puts("applying changes #{change_set.id}")
        ...
        change_details.changes.each do |change|
          ...
          puts("#{change.type} - #{resource_change.action} - #{resource_change.logical_resource_id} - #{resource_change.resource_type} - is replacement #{resource_change.replacement.nil? ? false : resource_change.replacement}")
          puts("change details: #{resource_change.details}")
        end
